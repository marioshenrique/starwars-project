...
from schemas.swapi_schemas import SpecieExternalSchema, SpeciesExternalSchema, FilmExternalSchema, PersonExternalSchema
...
async def get_films_by_specie(specie_id: int):
    url = f"{ENDPOINT_API_URL}/{specie_id}/"
    data = await get_correlated_data(url=url, data_label="films", main_model=SpecieExternalSchema, related_model=FilmExternalSchema)
    return data


async def get_people_by_specie(specie_id: int):
    url = f"{ENDPOINT_API_URL}/{specie_id}/"
    data = await get_correlated_data(url=url, data_label="people", main_model=SpecieExternalSchema, related_model=PersonExternalSchema)
    return data
